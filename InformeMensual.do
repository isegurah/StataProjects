...
import excel "C:\Reporte_mensual\Bases\CUBO_MENSUAL.xlsx", sheet("enero-septiembre") firstrow
...
clear
import excel "C:\Reporte_mensual\Bases\EMPRESAS_HISTORICO.xlsx", sheet("Sheet1") firstrow
...
**-----------------------Adecuaciones de variables-----------** 
	...
	**Comprime el archivo**
	 ...
	 **Renombrar la variable**
	 ...
	 keep if inlist(anio_termina,2020,2021)
	 tab anio_termina
...
gen capacitandos20=1 if anio_termina==2020 & periodo>0

mvencode capacitandos21-capacitandos20, mv(0)

tabstat capacitandos21, by(MES) s(sum)
tabstat capacitandos20, by(MES) s(sum)
...
gen persPeriodo20=1 if persPeriodo==1 & anio_termina==2020 & periodo>0

mvencode persPeriodo21 persPeriodo20 , mv(0)

tabstat persPeriodo21, by(MES) s(sum)
tabstat persPeriodo20, by(MES) s(sum)
...
gen persPeriodoPL20=1 if persPeriodoPL==1 & anio_termina==2020 & periodo>0

mvencode persPeriodoPL21 persPeriodoPL20 , mv(0)

tabstat persPeriodoPL21, by(PLANTEL) s(sum)
tabstat persPeriodoPL20, by(PLANTEL) s(sum)
...
gen cursos20=1 if cursos==1 & anio_termina==2020 & periodo>0

mvencode cursos21 cursos20 , mv(0)

tabstat cursos21, by(MES) s(sum)
tabstat cursos20, by(MES) s(sum)

**Horas Curso**
 gen horas21=duracion_horas if cursos21==1
 gen horas20=duracion_horas if cursos20==1
 
 mvencode horas21 horas20 , mv(0)

 tabstat horas21, by(MES) s(sum)
 tabstat horas20, by(MES) s(sum)

 **Horas Personas**
 gen horas_per21=duracion_horas if anio_termina==2021 & periodo>0
 gen horas_per20=duracion_horas if anio_termina==2020 & periodo>0
 tabstat horas_per21, by(MES) s(sum)
 tabstat horas_per20, by(MES) s(sum)
 
 **Aprobados**
 gen cap_apro21=1 if capacitandos21==1 & ESTATUS_ALUMNO=="APROBADO"
 gen cap_apro20=1 if capacitandos20==1 & ESTATUS_ALUMNO=="APROBADO"
 
 mvencode cap_apro21 cap_apro20 , mv(0)

 tabstat cap_apro21, by(MES) s(sum)
 tabstat cap_apro20, by(MES) s(sum)
 
 **Guardamos una archivo para el calculo por mes**
 save"base_mes.dta", replace
 
 collapse (sum) capacitandos21 capacitandos20 persPeriodoPL21 persPeriodoPL20 cursos21 cursos20 cap_apro21 cap_apro20 horas21 horas20 horas_per21 horas_per20, by(clv_plan PLANTEL)

 order clv_plan
 
 **Eficiencia terminal** 
 gen eficiencia21=cap_apro21/capacitandos21
 gen eficiencia20=cap_apro20/capacitandos20
 
 **Capacitandos por Persona**
 gen cap_per21=capacitandos21/persPeriodoPL21
 gen cap_per20=capacitandos20/persPeriodoPL20
 
 **Diferencias**
 gen cap_dif=capacitandos21-capacitandos20
 gen per_dif=persPeriodoPL21-persPeriodoPL20
 gen cur_dif=cursos21-cursos20
 gen hor_dif=horas21-horas20
 gen efic_dif=eficiencia21-eficiencia20
 gen cap_per_dif=cap_per21-cap_per20
  
order clv_plan PLANTEL capacitandos21 capacitandos20 cap_dif persPeriodoPL21 persPeriodoPL20 per_dif cursos21 cursos20 cur_dif horas21 horas20 hor_dif cap_apro21 cap_apro20 eficiencia21 eficiencia20 efic_dif  horas_per21 horas_per20 cap_per20 cap_per21 cap_per_dif
 ...
 **Guardar como .dta**
  ...
  **Reorganizamos la base por mes*
  collapse (sum) capacitandos21 capacitandos20 persPeriodo21 persPeriodo20 cursos21 cursos20 horas21 horas20 horas_per21 horas_per20 cap_apro21 cap_apro20, by(MES MESTERMINA)
 ...
 **Crear variables para ingresos 19-20**
 gen ingresos21=ingresos if anio_termina==2021 & periodo>0
 gen ingresos20=ingresos if anio_termina==2020 & periodo>0
 
 mvencode ingresos21 ingresos20 , mv(0)
  
 **Colapsar la base**
 collapse (sum) ingresos21 ingresos20, by(MES)
 
 joinby MES using  "comparativo_mes.dta", unmatched(master)
 
 order ingresos21, after(cap_apro20)
 order ingresos20, after(ingresos21)
...
import excel "C:\Reporte_mensual\Bases\INGRESOS_MENSUAL.xlsx", sheet("Comparativo ago") cellrange(A4:G36) firstrow
...
collapse (sum) capacitandos, by(id_empresa CADENA_PRODUCTIVA empresa)
...
collapse (sum) ingresos-cursos if periodo>0, by(PLANTEL)
...
gen clv_plan=1
		...
		replace clv_plan=128 if PLANTEL=="LEÓN"
		replace clv_plan=129 if PLANTEL=="SALAMANCA ALTA ESPECIALIDAD"
		replace clv_plan=131 if PLANTEL=="INSTITUTO DE EDUCACIÓN Y DESARROLLO RICHARD E. DAUCH"
		replace clv_plan=132 if PLANTEL=="PURÍSIMA DEL RINCÓN"
		replace clv_plan=133 if PLANTEL=="PUERTO INTERIOR"
...
collapse (sum) capacitandos persPeriodo cursos if periodo==1, by(SECTOR anio_termina)
